...
This combination of letters is used to generate a number 0:63. This is done by setting R=0, T=1, S=2 and P=3 and interpret it as a number on base 4. This will result in the next formula: points round 1 * 4^2 + points round 2 *4^1 + points round 3 * 4^0. SSP for instance is 43 is this way. Each of the numbers 0:63 either stands for defect or cooperate, therefore based on the last 3 moves a option is selected. Since this decision is based upon the last the 3 moves, the first three moves cannot be defined by this strategy. For this reason these are defected.
...
The string that was created from these scripts is CCCDCCCDCCCDCCCDCDCDCDDDCDDDDDDDCCCDCDDDCDDDDDDDDDDDCDCDCDDDDDDD. The agent will calculate the number that is in accordance with the last three meetings, this is the index for which number to take from the string.
...
library(R6)
Agent <- R6Class("Agent",
    #first we create different empty variables which are used by the agent during every round
    public = list(
    bid = NULL,
    book = NULL,
    greeting = NULL,
    ...
    #than we set some of these empty variables to standard information we get from the tournament about every round
    ...
    #the get_bid function decides what the agent does
    get_bid = function() {
        bid_vector <- c("cooperate","defect")
        #here the agent checks the last three rounds it had played with this agent.
        last_round <- tail(subset(self$book, (id1 == self$id | id2 == self$id) & (id1 == self$opponent_id | id2 == self$opponent_id)), 3)
        #this is the created string that was made from the data from last year. Because of coding troubles it is a vector and not a string.
        ...
        #there're agents who only defect in the tournament. Their greeting is "Lemon!", because the only answer to defect is to defect as well, the agent checks the greeting of the opponent.
...
Our first agent had different functions which were called by the get_bid function, but the provided tournament code didn't work with this agent. Therefore we were forced to put all the functions in one large function. With further testing we discovered that other parts of the code that worked sepparatly, didn't work with the provided tournament code. This let us to change our code even more.
